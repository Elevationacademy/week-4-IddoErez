...
let mailCname = users.map(u => {return {email: u.email, companyName: u.company.name}})
    console.log(mailCname)

//--Exercise 2 -- 

 let startsWithFive = users.filter(u => u.address.zipcode.charAt(0) === "5")  
  console.log(startsWithFive)

  //--Exercise 3 --
  let getThreeIds = startsWithFive.map(s =>s.id)
  console.log(getThreeIds)

  //--Exercise 4 --
  let names = users.map(u => u.name)
  let startsWithC = names.filter(n => n.startsWith('C') === true)
  console.log(startsWithC)

//-Exercise 5 --
let liveInSouthChristy = users.every(u=> u.address.city === "South Christy")
console.log(liveInSouthChristy)
//--Exercise 6 --
    console.log(users.find(u => u.address.suite === "Apt. 950").company.name)
//--Exercise 7 --
const findUser = (user) => {
   return (u=> console.log(`${u.name} lives in ${u.address.city}, and owns the company ${u.company.name}`))

}
const userCompany = users.forEach(findUser())
...
// let lowerMessage =  messagesFromDad.map(mDad=> mDad.toLocaleLowerCase())
...
//     return posts[postID].comments.find(c=> c.id === commentID)
...
//     { title: "Thor: Ragnarok", studio: "Marvel", year: 2021 },
// ]
...
// // const commentID =posts[id] => posts[id].comments.find (c=>posts.id)
